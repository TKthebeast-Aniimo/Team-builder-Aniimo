...
/*
   ...
   Official exact curves are not fully published.
   Higher IP+AP increases attribute gain with level.
   We model potential as a growth multiplier:

       mult = 1 + ((IP + AP) / 20) * 0.35

   So perfect 20/20 is ~+35% over base growth at the
   same level. Blank/0 leaves base stats unchanged
   aside from level scaling and capability/personality.
*/
...
function computeFinalStats(aniimo, activeData) {
    ...
    STAT_ORDER.forEach(
        function (stat) {
            ...
            /*
               Level growth with potential:
               base * (1 + (level-1)*0.02 * potMult)
            */
            let value =
                trueBase *
                (
